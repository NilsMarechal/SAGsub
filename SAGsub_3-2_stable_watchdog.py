#!/usr/bin/env python3
import os, sys, glob, mrcfile, queue, time
import shutil as sh
...
def on_created(event):
    q.put(os.path.relpath(event.src_path,workPath))
...
if __name__ == '__main__':
    ...
    for f in glob.glob('**/*.mrc', recursive=True):
        if 'aligned_doseweighted' in f:
            ...
            ori_list = glob.glob('**/*doseweighted.mrc', recursive=True)
            patterns = ["*doseweighted.mrc"]
            if '_sub.mrc' in f:
                continue
        elif 'background' in f or 'aligned.mrc' in f:
            continue
        else:
            preprocess_type = 'warp'
            ori_list = glob.glob('**/*.mrc', recursive=True)
            patterns = ["*.mrc"]
    ...
    my_event_handler.on_deleted = on_else
    ...
    my_observer.schedule(my_event_handler, workPath, recursive=True)
