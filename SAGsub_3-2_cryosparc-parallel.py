...
if __name__ == '__main__':
    ...
    parser.add_argument("cur_job", help="Folder name of Curate Exposures job \(for e.g. J4\)")
    parser.add_argument("-n","--num_proc", type=int, default=16, help="Number of parallel processes")
    args = parser.parse_args()
    if os.path.isfile(args.cur_job + '/good_micrographs-curated.star'):
        curStar = open(args.cur_job + '/good_micrographs-curated.star','r')
    else:
        print('good_micrographs-curated.star file was not found in the folder, please run curate2star.py first! - exiting')
        sys.exit()
    for line in curStar:
        if 'patch_aligned_doseweighted.mrc' in line:
            firstMrc = mrcfile.open(line.split(' ')[0])
