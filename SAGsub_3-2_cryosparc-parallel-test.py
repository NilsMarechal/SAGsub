#!/usr/bin/env python3
import os, sys, glob, mrcfile, argparse, sqlite3
import shutil as sh
import numpy as np
...
#            spot_res   = Spot_MetaData[k][1]
    ...
    #log.write('\n')
    params = (LOG[0], LOG[1], LOG[2], LOG[3])
    curs.execute("INSERT INTO Details VALUES (NULL, ?, ?, ?, ?)", params)
    con.commit()
...
if __name__ == '__main__':
    ...
    if os.path.exists('./micdets.db'):
        os.remove('./micdets.db')
    con = sqlite3.connect('./micdets.db')
    curs = con.cursor()
    curs.execute('''CREATE TABLE Details
        (ID      INTEGER PRIMARY KEY    AUTOINCREMENT,
        IMA_NAME           TEXT    NOT NULL,
        Mosaicity          REAL    NOT NULL,
        SpotErr            REAL    NOT NULL,
    ...
    mic_list = []
    for image in orri_list:
        if image.replace('.mrc','_sub.mrc') not in sub_list:
            mic_list.append(image)
    ...
    if len(sub_list) == 0:
        ...
        log = open('SAGsub_3.star','a')
    
    with Pool(args.num_proc) as pool:
        results = pool.map(subract_lattice, mic_list)
    print('Ready - writing log')

    curs.execute("SELECT * FROM Details")
    log_details = curs.fetchall()
        
    log.write('\n'.join('{}\t{}\t{}\t{}'.format(x[1],x[2],x[3],x[4]) for x in log_details))
    log.write('\n')
