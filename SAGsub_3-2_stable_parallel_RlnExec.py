#!/usr/bin/env python3
import os, sys, glob, mrcfile, argparse
import shutil as sh
import numpy as np
...
def subract_lattice(mic):
    LOG = [None]*4
    try:
        mrc.read(mic)                                   #mrc object has to be declared!
        LOG[0] = mic
    ...
    params = (str(LOG[0])+'\t'+str(LOG[1])+'\t'+str(LOG[2])+'\t'+str(LOG[3]))
    return params

def consolidate_starfile(star_input, star_sub):
    input_starfile = starfile.read(star_input)
    subtracted_mics = []
    with open(star_sub, 'r') as file:
        for row in file:
           if not row.startswith('_rln') and "SUCCESS" in row:
               subtracted_mics.append(row.split('\t')[0])
    micrographs_out = input_starfile['micrographs'][input_starfile['micrographs']['rlnMicrographName'].isin(subtracted_mics)]
    micrographs_out['rlnMicrographName'] = micrographs_out['rlnMicrographName'].str.replace('.mrc', '_sub.mrc')
    star_out = {'optics':input_starfile['optics'], 'micrographs':micrographs_out}
    starfile.write(star_out, args.output+'subtracted_micrographs.star')
    

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Subtracts strep lattice from Motion-corrected micrographs from RELION")
    parser.add_argument("-o", "--output" , "--o", help = "output folder")
    parser.add_argument("-i", "--in_mics", help = "input CTF micrograph starfile")
    parser.add_argument("-j","--j", type=int, default=4, help="Number of parallel processes")
    args, unknown = parser.parse_known_args()

    input_mics = starfile.read(args.in_mics)['micrographs']['rlnMicrographName']

    firstMrc = mrcfile.open(input_mics[0])
    pixSize = firstMrc.voxel_size.field(0)
    ...
    done_mic_list = []
    for i in input_mics:
        if i in glob.glob(args.output, recursive=True):
            done_mic_list.append(i)
            continue
        else:
            mic_list.append(i)
    ...
    if len(done_mic_list) == 0:
        log = open(args.output+'SAGsub_3.star','w')
        log.write('_loop\n\n_rlnMicrographName #1\n_rlnMosaicity #2\n_rlnSpotDistError #3\n_rlnStatus #4\n')
    else:
        log = open(args.output+'SAGsub_3.star','a')
    
    pool = Pool(args.j)
    for result in pool.imap_unordered(subract_lattice, mic_list):
        log.write(result +'\n')
        log.flush()
    
    consolidate_starfile(args.in_mics, args.output+'SAGsub_3.star')

    end_file=open(args.output+"RELION_JOB_EXIT_SUCCESS","w+")
    end_file.close()
